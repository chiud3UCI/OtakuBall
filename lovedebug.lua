--This has been modified to be compatible with version 11
--apparently love.filesystem.getLastModified got deprecated
local function getLastModified(file)
	info = love.filesystem.getInfo(file)
	return info.modtime
end

--and now they have to mess up the setColor() function
local function legacyGetColor()
...
local _Debug = {
	...
	liveOutput='',
	liveLastModified=getLastModified('main.lua'),
	liveDo=false
}
...
_Debug.onTop = function()
	...
	if #p > 0 then
		legacySetColor(127, 127, 127, 255)
		...
		if liveflag then -- Setting up lastModified for the new live file(s) if changed
			if type(_DebugSettings.LiveFile) == 'table' then
				...
				for i = 1, #_DebugSettings.LiveFile do --Setting up lastModified for live files
					...
					_Debug.liveLastModified[i] = getLastModified(_DebugSettings.LiveFile[i])
				end
			else
				if love.filesystem.getInfo(_DebugSettings.LiveFile) then
					_Debug.liveLastModified = getLastModified(_DebugSettings.LiveFile)
				else
...
--Handle Keypresses
--modified so the tilde(`) key can toggle the overlay as well.
_Debug.handleKey = function(a)
	--local activekey = love.system.getOS()~='Android' and (_lovedebugpresskey or "f8") or 'menu'
	if a == "`" or (love.keyboard.isDown("lshift", "rshift", "lctrl", "rctrl") and a == "f8") then
...
_Debug.liveCheckLastModified = function(table1,table2)
	if type(table1) == 'string' then
		if getLastModified(table1) ~= table2 then
			return true
		end
		return false
	end
	...
	for i,v in ipairs(table1) do
		if getLastModified(v) ~= table2[i] then
			return true
		end
	end
	return false
...
_G["love"].run = function()
	...
	while true do
		...
		if _Debug.drawOverlay then
			for key, d in pairs(_Debug.trackKeys) do
				if type(key) == 'string' then
					if key == " " then key = "space" end --love 10.0 space keycode fix
			...
			if _DebugSettings.LiveAuto and _Debug.liveCheckLastModified(_DebugSettings.LiveFile,_Debug.liveLastModified) then
				if type(_DebugSettings.LiveFile) == 'table' then
					for i=1,#_DebugSettings.LiveFile do
						if getLastModified(_DebugSettings.LiveFile[i]) ~= _Debug.liveLastModified[i] then
		...
		elseif love.update and (_Debug.liveDo or (_DebugSettings.LiveAuto and _Debug.liveCheckLastModified(_DebugSettings.LiveFile,_Debug.liveLastModified))) then
			if type(_DebugSettings.LiveFile) == 'table' then
				for i=1,#_DebugSettings.LiveFile do
					if (_DebugSettings.LiveAuto and getLastModified(_DebugSettings.LiveFile[i]) ~= _Debug.liveLastModified[i]) or _Debug.liveDo then
						_Debug.hotSwapUpdate(dt,_DebugSettings.LiveFile[i])
						_Debug.liveLastModified[i] = getLastModified(_DebugSettings.LiveFile[i])
					end
				end
